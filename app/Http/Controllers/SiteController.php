...
class SiteController extends Controller
{
    ...
    public function index(){
        $count = Page::where('slug','home')->count();
        if($count == 0){
            $page = new Page($in);
            $page->name = 'HOME';
            $page->slug = 'home';
        ...
        $data['sections'] = Page::where('slug','home')->firstOrFail();

        $plan = DB::table('ico_plans')->whereDate('end_date', '>=', Carbon::now())->first();
